...
export async function curateLeads(cands: LeadCand[]): Promise<Scored[]> {
  ...
  const system =
    ...
    "수도권의 '프랜차이즈가 아닌' 1~2호점 규모 베이커리·디저트·떡집 중, 리뷰 많고 유명하거나 입소문 타는 전문점을 찾아 공동구매 컨택 대상으로 점수화해. " +
    "이미 유행이 굳어 '곤조 센'(납품 거절할) 곳은 감점, 요즘 뜨는 신흥 전문점은 가점. 대형 프랜차이즈는 제외 대상. " +
    "각 후보를 0~100 점수와 한국어 한 줄 이유로. 반드시 입력 순서대로 JSON 배열만 출력: [{\"score\":0-100,\"reason\":\"...\"}]";
  const user = cands
