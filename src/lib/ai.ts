...
- 수량: 적힌 그대로(숫자는 임의 변경·추정 금지, 없으면 ""). 숫자만이면 숫자만("사과 3"→qty "3"), 단위가 있으면 그 단위 유지("20박스"→"20박스", "딸기 10kg"→"10kg", "대파 5단"→"5단"). 없는 단위(박스 등)를 임의로 붙이지 마세요. "N다이"(4다이·9다이)는 박스 속 낱개 개수=크기/등급이지 수량이 아니므로 qty에 그대로 두고 합산하지 마세요.
