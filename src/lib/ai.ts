...
- [수량 단위] 카테고리가 '과일' 또는 '야채'일 때, 수량에 단위 없이 숫자만 있으면 '박스'로 봅니다. 예) "3"→"3박스", "사과 2"→qty "2박스". kg·g 등 무게 단위는 그대로 둡니다. 예) "딸기 10kg"→"10kg". 이미 단위(다이/박스/개/알/단/봉지/kg 등)가 있으면 그대로 유지. (과일·야채가 아니면 단위를 임의로 붙이지 마세요.)
...
- 수량: 숫자는 임의 변경·추정 금지(없으면 ""). 과일·야채로 분류된 품목은 단위 없이 숫자만이면 '박스'로(예: "사과 3"→qty "3박스"). kg·g 무게는 그대로("딸기 10kg"→"10kg"). 이미 단위(다이/박스/개/단/봉지/kg 등)가 있으면 유지.
...
- 같은 항목 합산(단위 일관 시). 단위가 섞이면 "11다이 4건"처럼 간단히.
