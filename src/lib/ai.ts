...
[임무 3 · 카테고리별 교정 강도 · 매우 중요]
- category가 '공구'(공동구매)면: **품목명을 절대 바꾸지 마세요.** 밀키트·가공식품(소머리곰탕·낙지·케이크 등)은 만든 업체가 붙인 고유 상품명이라 '표준명'이 없습니다. 오타 교정조차 하지 말고 적힌 그대로 두세요. 예) "세월낙지"를 "세발낙지"로 바꾸면 안 됩니다(❌ 발주 사고). "부산 세월낙지"도 그대로.
- category가 '야채'면: 흔한 채소명의 오타·소리나는 대로 적은 것을 표준 채소명으로 적극 교정하세요(발음이 비슷한 명백한 오타). 예) "영배추"·"양베추"·"양배차"→"양배추", "데파"→"대파", "부초"·"부주"→"부추", "시굼치"→"시금치", "상치"→"상추", "깨잎"→"깻잎", "얼가리"→"얼갈이", "미나이"→"미나리", "마늘쫑"→"마늘종", "콩너물"→"콩나물". 단, 실제로 없는 이름이거나 어느 채소인지 불확실하면 추측하지 말고 원문 그대로 두세요.
- category가 '과일'일 때만 위 [임무1]의 품종 교정을 적극 적용하세요.
...
- 특히 채소(야채)는 흔한 오타를 적극 교정하세요: "영배추"·"양베추"·"양배차"→"양배추", "데파"→"대파", "부초"→"부추", "시굼치"→"시금치", "상치"→"상추", "깨잎"→"깻잎", "얼가리"→"얼갈이", "마늘쫑"→"마늘종", "콩너물"→"콩나물". 없는 이름·불확실한 건 원문 유지.
- 단, 공구(TOOL)로 분류된 품목은 상품명을 **절대 바꾸지 마세요.** 밀키트·가공식품은 업체 고유명이라 표준명이 없어요(예: "세월낙지"를 "세발낙지"로 바꾸면 안 됨). 적힌 그대로 두세요.
...
3) 각 종류마다: 지점별 내역(lines: store·qty·note). qty는 '숫자'만 들어옵니다. 같은 종류면 숫자를 더해 total(숫자)로 내세요(예 3+5=8). 수량이 아예 없는 줄은 빼고, 애매하면 "".
