...
// ── 오타 수준 변화만 수용하는 결정론적 가드 ───────────────────────────────
// AI가 멀쩡한 이름을 '가장 가까운 다른 품목'으로 갈아치우거나(미니 오이→비포 오이)
// 수식어를 떨구는(빨간 사과→사과) 문제를 원천 차단한다. 원문과 AI 제안의 '자모 편집거리'가
// 오타 수준(작을 때)만 AI 이름을 쓰고, 그보다 크면 원문 이름을 그대로 보존한다.
// → 프롬프트(비결정)와 무관하게 "오타만 교정"을 보장. 순수 함수라 단위 검증 가능.
const HANGUL_BASE = 0xac00;
const L_JAMO = "ㄱㄲㄴㄷㄸㄹㅁㅂㅃㅅㅆㅇㅈㅉㅊㅋㅌㅍㅎ";
const V_JAMO = "ㅏㅐㅑㅒㅓㅔㅕㅖㅗㅘㅙㅚㅛㅜㅝㅞㅟㅠㅡㅢㅣ";
const T_JAMO = " ㄱㄲㄳㄴㄵㄶㄷㄹㄺㄻㄼㄽㄾㄿㅀㅁㅂㅄㅅㅆㅇㅈㅊㅋㅌㅍㅎ";

// 한글 문자열을 자모(초·중·종성) 토큰 배열로 분해. 한글이 아니면 글자 그대로.
function toJamo(s: string): string[] {
  const out: string[] = [];
  for (const ch of s.replace(/\s+/g, "")) {
    const code = ch.charCodeAt(0) - HANGUL_BASE;
    if (code >= 0 && code < 11172) {
      out.push(L_JAMO[Math.floor(code / 588)], V_JAMO[Math.floor((code % 588) / 28)]);
      const t = code % 28;
      if (t > 0) out.push(T_JAMO[t]);
    } else {
      out.push(ch);
    }
  }
...
// 원문(orig) → AI 제안(ai) 이름이 '오타 수준'의 작은 변화인가.
// true면 AI 이름 수용(오타 교정), false면 원문 이름 보존(임의 치환·수식어 누락 차단).
export function isTypoLevelChange(orig: string, ai: string): boolean {
  const o = orig.replace(/\s+/g, "");
  const a = ai.replace(/\s+/g, "");
  if (!o || !a) return false;
  if (o === a) return true;
  // 글자 수가 크게 줄면(수식어 누락: '빨간 사과'→'사과') 거부
  if (a.length < o.length * 0.6) return false;
  const jo = toJamo(o);
  const ja = toJamo(a);
  const d = jamoEditDistance(jo, ja);
  const lenMax = Math.max(jo.length, ja.length);
  // 자모 편집거리가 전체 길이의 ~25% 이내(최소 1)일 때만 오타로 인정
  return d <= Math.max(1, Math.floor(lenMax * 0.25));
}

// 서부일광(도매) 실제 취급 품목 + 표준 품종 사전. '명백한 오탈자를 바로잡을 때의 참고 후보'.
// 사전에 비슷하다고 강제 치환하지 않는다(원문 보존 원칙). 최종적으로 큰 변화는 isTypoLevelChange가 되돌린다.
...
- 단호박·고구마·밤·대추·건대추·야자대추 (※ 고구마는 품종 무관 항상 '과일'로 분류 — 자색·밤·호박·베니지민 고구마 포함)
■ 채소
- 대파·쪽파·양파·마늘·마늘종·생강·당근·감자·무
- 배추·알배추·양배추·상추·깻잎·시금치·부추·얼갈이·미나리·쑥갓·근대
- 콩나물·숙주·오이·애호박·고추·청양고추·파프리카·브로콜리·가지·버섯·옥수수
■ 특별 품종 · 베니지민 고구마(매우 중요)
- '베니지민'은 실재하는 고구마 품종입니다. 소리나는 대로 적어 오타가 잦아요.
  오타 예) "배니지민"·"베나지민"·"베지니민"·"베니지믄"·"베니진민"·"베니 고구마" → 모두 "베니지민"으로 교정.
- 베니지민 고구마는 name과 note를 반드시 '분리'해서 정리하세요: **name은 "고구마", note에 "베니지민"**.
  · 절대 name에 "베니지민 고구마"처럼 합치지 마세요. 항상 name="고구마", note="베니지민".
  · 품목명이 "고구마"이고 note(설명)에 '베니지민'류가 있으면 그대로 name="고구마", note="베니지민"으로 두세요(합치기 금지).
  · name에 "베니지민"만 있고 "고구마"가 없어도 name="고구마", note="베니지민"으로 분리하세요.
■ 우리 매장 실제 취급 야채(되도록 이 이름으로 맞추세요 · 조은팜 기준)
${DS_VEG.join("·")}
■ 우리 매장 실제 취급 과일(서부일광 기준)
${DS_FRUIT.join("·")}
[매장 품목 매핑 · 참고만] 위 '매장 취급' 목록은 '명백한 오탈자를 바로잡을 때의 참고 후보'일 뿐입니다. 비슷하다고 해서 원문을 목록의 이름으로 강제로 바꾸지 마세요 — 명백한 오탈자만 최소로 바로잡고, 그 외에는(수식어 포함) 적힌 그대로 두세요.`;
...
[가장 중요 · 원문 보존이 기본]
- 점주가 적은 품목명은 '적힌 그대로' 두는 것이 원칙입니다. 다른 품목으로 바꾸거나, 수식어(미니·빨간·조선·초당·알타리·햇 등 크기·색·품종·상태를 나타내는 말)를 절대 빼지 마세요.
  · 나쁜 예: "미니 오이"→"비포 오이"(❌ 전혀 다른 말로 치환), "빨간 사과"→"사과"(❌ 수식어 누락). 이런 임의 변경 절대 금지.
- 사전/매장 목록의 '가장 가까운 다른 품목'으로 끼워맞추지 마세요. 사전은 오타를 바로잡을 때 '참고'만 하고, 그 이름으로 강제 치환하지 않습니다.

[임무 1 · 명백한 오타만 최소 교정]
- category가 '과일'/'야채'일 때, 자모(초·중·종성)가 대부분 일치하고 의도가 분명한 '오탈자'만 표준 표기로 최소한으로 바로잡으세요.
  과일 예) "방울도마도"→"방울토마토", "파인에플"→"파인애플", "딸끼"→"딸기"
  야채 예) "당그은"→"당근", "데파"→"대파", "깨잎"→"깻잎", "콩너물"→"콩나물", "마늘쫑"→"마늘종"
- 조금이라도 확신이 없으면 원문 그대로 두세요. 억지 교정·비약·창작 금지, 이미 올바른 이름은 절대 바꾸지 마세요("시나노골드"→"시나몬골드"❌).
- '인식불가' 같은 표시도 하지 말고, 못 알아봐도 원문을 그대로 두세요(관리자가 확인합니다).
...
- 부연설명(note)은 '짧은 명사 태그' 하나로만. 문장·존댓말 금지(자잘한 설명 X).
  ...
  · 단, 색·크기·품종·상태 수식어(빨간·미니·조선·초당·햇 등)는 '품목 이름의 일부'이니 note로 옮기지 말고 name에 그대로 두세요.
...
async function claudeNormalize(input: NormalizeInput): Promise<NormalizeResult> {
  ...
  // 결정론적 가드: AI 이름이 원문의 '오타 수준' 변화일 때만 수용, 아니면 원문 이름/비고 보존.
  // (미니 오이→비포 오이, 빨간 사과→사과 같은 임의 치환·수식어 누락 차단. 순서·개수는 위에서 1:1 보장.)
  const items: CleanItem[] = (parsed.items ?? []).map((it, i) => {
    const origName = tidy(input.items[i]?.name ?? "");
    const aiName = tidy(it.name ?? "");
    const keepAi = isTypoLevelChange(origName, aiName);
    return {
      name: keepAi ? aiName : origName,
      qty: tidy(it.qty ?? ""),
      note: keepAi ? tidy(it.note ?? "") : tidy(input.items[i]?.note ?? ""),
    };
  });
...
${catLines}
  ...
  · 고구마(자색·밤·호박·베니지민 등 모든 품종) = FRUIT(과일) — 무조건 과일로.
...
[정규화 · 원문 보존이 기본]
- 적힌 품목명을 '그대로' 두는 것이 원칙입니다. 다른 품목으로 바꾸거나 수식어(미니·빨간·조선·초당·햇 등 크기·색·품종)를 빼지 마세요. (나쁜 예: "미니 오이"→"비포 오이"❌, "빨간 사과"→"사과"❌)
- 자모가 대부분 일치하고 의도가 분명한 '명백한 오탈자'만 최소한으로 바로잡으세요. 예) "방울도마도"→"방울토마토", "당그은"→"당근", "데파"→"대파", "깨잎"→"깻잎", "콩너물"→"콩나물". 조금이라도 확신이 없으면 원문 그대로.
- 사전/매장 목록의 '가장 가까운 다른 품목'으로 끼워맞추지 말고(참고만), 없는 단어를 지어내지 마세요. '인식불가' 표시도 하지 말고 못 알아봐도 원문 그대로 두세요.
