...
- [수량 단위] 카테고리가 '과일' 또는 '야채'일 때, 수량에 단위 없이 숫자만 있으면 '박스'로 봅니다. 예) "3"→"3박스", "사과 2"→qty "2박스". kg·g 등 무게 단위는 그대로 둡니다. 예) "딸기 10kg"→"10kg". 이미 단위(박스/개/알/단/봉지/kg 등)가 있으면 그대로 유지. (과일·야채가 아니면 단위를 임의로 붙이지 마세요.)
- [다이는 수량이 아님 · 매우 중요] "N다이"(예: 4다이·9다이)는 '한 박스 안에 든 낱개 개수 = 크기/등급' 표기예요. 주문 수량(박스 수)이 아닙니다. 다이 숫자를 수량으로 바꾸거나 박스로 환산하지 말고, qty에 "4다이"처럼 적힌 그대로 두세요. (예: "부사 4다이"→qty "4다이". 서로 다른 다이는 다른 크기이므로 절대 합산하지 마세요.)
...
- 수량: 숫자는 임의 변경·추정 금지(없으면 ""). 과일·야채로 분류된 품목은 단위 없이 숫자만이면 '박스'로(예: "사과 3"→qty "3박스"). kg·g 무게는 그대로("딸기 10kg"→"10kg"). 이미 단위(박스/개/단/봉지/kg 등)가 있으면 유지. "N다이"(4다이·9다이)는 박스 속 낱개 개수=크기/등급 표기지 수량이 아니므로 qty에 그대로 두고 박스로 바꾸거나 합산하지 마세요.
...
[다이는 수량 아님] "N다이"(4다이·9다이)는 박스 속 낱개 개수=크기/등급이지 주문 수량이 아닙니다. 절대 합산하지 마세요(3다이와 5다이는 서로 다른 크기 → 합쳐서 8다이로 만들지 말 것). 다이가 섞인 종류의 total은 ""로 두고, 각 지점 qty의 다이 표기는 그대로 유지하세요.
...
- 품목/종류별로 한 줄씩. 형식: "[항목] — [수량] 이상 입찰 준비"
  예) "부사 사과 — 9다이 이상 입찰 준비", "샤인머스캣 — 4다이 입찰 준비", "감귤 — 5다이 입찰 준비"
- 등급/요청(특·고당도·행사 등)이 있으면 항목 뒤 괄호로: "부사 사과(특) — 3다이 입찰 준비".
- 같은 항목은 진짜 수량 단위(박스·개·단·kg)가 같을 때만 합산. "다이"는 한 박스 속 낱개 개수=크기/등급이라 수량이 아니므로 합산하지 말 것 — 다이가 다르면 등급별로 한 줄씩(예: 위처럼 "9다이"/"4다이"는 그 등급의 입찰 기준), 합치고 싶으면 "N건"으로만.
- 과일 분류 정확히: 부사·홍로·시나노골드=사과, 거봉·샤인머스캣·어텀크리스피=포도, 한라봉·천혜향=감귤.
- 같은 과일끼리 인접하도록 정렬.

결과는 순수 JSON만:
{"sentences":["부사 사과(특) — 3다이 이상 입찰 준비","부사 사과 — 6다이 이상 입찰 준비","시나노골드 — 3박스 입찰 준비","샤인머스캣 — 4다이 입찰 준비"]}`;
