...
export default async function ReservationInvoiceDetailPage({
  params,
}: {
  ...
  if (!inv || inv.userId !== user.id || inv.kind !== "RESERVATION") notFound();
  ...
  return (
    <>
      <Topbar backHref="/reservations/invoices" title="예약발주 입금요청서" right={<TopbarChip>{user.storeName}</TopbarChip>} />
      <div className="page">
        ...
        <p className="lead">{labelDateLong(inv.date)} 픽업</p>
